...
class Trainer():
    ...
    def init_GAN(self):
        ...
        if self.switched_conv and self.init_swconv:
            self.GAN = SwitchedConvConversionWrapper(self.GAN, 8, ['G.layers.3.0.2', 'G.layers.4.0.2', #'G.out_conv', 'G.layers.5.0.2',
                                                                        ...
                                                                        ], coupler_mode='lambda', dropout_rate=0)
    ...
    def save(self, num):
        save_data = {
            ...
            'optD': self.GAN.D_opt.state_dict(),
            'optG': self.GAN.G_opt.state_dict()
        }
    ...
    def load(self, num=-1, print_version=True):
        ...
        if 'optG' in load_data:
            self.GAN.G_opt.load_state_dict(load_data['optG'])
        if 'optD' in load_data:
            self.GAN.D_opt.load_state_dict(load_data['optD'])
